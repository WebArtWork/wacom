...
export class CoreService {
	...
	constructor(@Inject(PLATFORM_ID) private platformId: boolean) {
		...
		const userAgent =
			this.navigator.userAgent || this.navigator.vendor || this.window.opera;
	...
	splice(removeArray: any[], fromArray: any[], compareField = '_id') {
		removeArray = Array.isArray(removeArray) ? removeArray.slice() : [];

		fromArray = Array.isArray(fromArray) ? fromArray.slice() : [];

		for (let i = fromArray.length - 1; i >= 0; i--) {
			if (
				removeArray.find((o) => o[compareField] === fromArray[i][compareField])
			) {
